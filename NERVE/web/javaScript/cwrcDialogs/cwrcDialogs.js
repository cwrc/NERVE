...
class CWRCDialogs{
    constructor(){
        Utility.log(CWRCDialogs, "constructor");
        Utility.enforceTypes(arguments);

        this.cwrcApi = "http://cwrc-dev-06.srv.ualberta.ca/islandora/cwrc_entities/v1/";
        this.repositoryBaseObjectURL = "http://cwrc-dev-01.srv.ualberta.ca/islandora/object/";
        this.geonameUrl = "http://cwrc-dev-06.srv.ualberta.ca/cwrc-mtp/geonames/";
        this.viafUrl = "http://cwrc-dev-06.srv.ualberta.ca/services/viaf/";
        this.googleGeocodeUrl = "http://maps.googleapis.com/maps/api/geocode/xml";
    }

    setCwrcApi(url){
        Utility.enforceTypes(arguments, String);
        this.cwrcApi = new CwrcApi(url, $);
    }

    setRepositoryBaseObjectURL(url){
        Utility.enforceTypes(arguments, String);
        this.repositoryBaseObjectURL = url;
    }
    ...
    setGoogleGeocodeUrl(url){
        Utility.enforceTypes(arguments, String);
        this.googleGeocodeUrl = url;
    }
}
...
//          search.isDataSelected(false); // setting to false here breaks modal dismiss
        ...
        search.runCustomAction = function (binding, event) {
            search.scrapeResult();
            custom.action(search.GetResult(), event);
            search.clear();
        }

        search.returnSelected = function (binding, event) {
            search.scrapeResult();
            search.success(search.GetResult(), event);
            search.clear();
        };

        search.cancel = function (binding, event) {
            if (search.cancelled) {
